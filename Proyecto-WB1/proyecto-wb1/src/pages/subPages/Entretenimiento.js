// src/pages/subPages/Entretenimiento.js
...
const Entretenimiento = () => {
    ...
    //const itemsCollectionRef = collection(db, "entretenimiento");
    ...
    const handleAddItem = async () => {
        if (newItem.name && newItem.price) {
            ...
            await addDoc(historialCollectionRef, { name: `Entretenimiento: ${newItem.name}`, amount: parseFloat(newItem.price), date: new Date() });
    ...
    const handlePriceChange = async (id, newPrice) => {
        const itemDoc = doc(db, "entretenimiento", id);
        await updateDoc(itemDoc, { price: parseFloat(newPrice) });
        await addDoc(historialCollectionRef, { name: `Actualización de Entretenimiento`, amount: parseFloat(newPrice), date: new Date() });
    ...
    return (
        <div>
            ...
            <Container className="mt-5 text-center">
                <h2>Lugares recreativos y su gasto (parques de diversiones, cine, teatro, bibliotecas)</h2>
